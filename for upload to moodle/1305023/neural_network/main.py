...
def getNN(xTrain, yTrain, neuronCntList, learningRate, totalRound):
    ...
    for roundIdx in range(totalRound):
        ...
        misclassifyInThisRound = 0
        overallOutputVecInThisRound = np.zeros( (outputVecSize, 1) )
        ...
        for exampleIdx in range(exampleCnt):
            ...
            curTrueY = yTrain[exampleIdx,:]
            curTureClass = curTrueY.argmax()
            ...
            curPredictedClass = estimatedY.argmax()
            overallOutputVecInThisRound[curPredictedClass] += 1


            if ( curTureClass != curPredictedClass ):
                misclassifyInThisRound += 1
            ...
            errorForThisExample = distSq(estimatedY, curTrueY)
        ...
        print("misclassifyInThisRound")
        print(misclassifyInThisRound)

        print("overallOutputVecInThisRound")
        print(overallOutputVecInThisRound)
...
def getW(neuronCntList, arrayGettingMethod):
    ...
    for i in range(1, len(neuronCntList) ):
        ...
        wr = scale * ( arrayGettingMethod((curLayerNeuronCnt, prevLayerNeuronCnt+1)) - 0.5 )
        w.append(wr)
...
pathToDataFileFromProjectRoot = "data2/train.txt";
dataSet = getDataSet(pathToFileFromProjectRoot=pathToDataFileFromProjectRoot)
ret = getXY(dataSet=dataSet, featureCnt=2, outputVecSize=3)
...
neuronCntList=[featureCnt, 5, 5, outputVecSize]
learningRate = 0.1
roundCnt = 10000
